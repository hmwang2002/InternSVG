...
S3_CONFIG = {
    "key": "cX0zA9eC0uO3eL3nZ3uW",
    "secret": "hG8vO3cW7iL2nM7gP3lP6tE4eY8kZ5",
    "endpoint": "http://10.135.4.29:8060",
    "region": "us-east-1",
}
...
def _get_s3fs():
    """创建/返回全局 s3fs 客户端。配置来自 S3_CONFIG 字典。"""
    global _S3FS
    if _S3FS is None:
        import s3fs
...
def _strip_xsky(u: str) -> str:
    return u[len("xsky:"):] if isinstance(u, str) and u.startswith("xsky:") else u

def open_image_any(p):
    """打开本地或 S3/Ceph 图片，统一返回 RGB 的 PIL.Image。支持:
       - 本地路径: '/path/to/xx.jpg'
       - 远端URL: 's3://bucket/key' 或 'xsky:s3://bucket/key'
       - 字节/文件对象: bytes / BytesIO / BinaryIO
    """
    # print("---->1111<----")
    from PIL import Image  # 延迟导入以保持与 is_pillow_available 逻辑一致
    # 字节或文件对象
    if hasattr(p, "read") or isinstance(p, (bytes, bytearray, BytesIO)):
        return Image.open(p).convert("RGB")

    # 远端 URL
    if isinstance(p, str) and p.startswith(("s3://", "xsky:s3://")):
        fs = _get_s3fs()
        url = _strip_xsky(p)
        # 规范为 s3://bucket/key
        norm = "s3://" + str(PurePosixPath(url[5:]))  # 去掉 's3://'
    ...
    def _regularize_images(self, images: list["ImageInput"], **kwargs) -> "RegularizedImageOutput":
        ...
        for image in images:
            if isinstance(image, (str, BinaryIO)):
                image = open_image_any(image)
            elif isinstance(image, bytes):
                image = open_image_any(BytesIO(image))
            elif isinstance(image, dict):
                if image.get("bytes") is not None:
                    image = open_image_any(BytesIO(image["bytes"]))
                else:
                    image = open_image_any(image["path"])
