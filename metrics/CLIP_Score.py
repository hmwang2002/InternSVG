...
class CLIPScoreCalculator(BaseMetric):
    ...
    def calculate_score(self, batch, batch_size=64, update=True):
        if self.task_type == 'T2I':
            ...
            data_loader = DataLoader(list(zip(pred_images, captions)), collate_fn=self.collate_fn, batch_size=batch_size, shuffle=False, num_workers=4, pin_memory=True) # type: ignore
        else:
            pred_images = batch['pred_im']
            gt_images = batch['gt_im']
            # Create DataLoader with custom collate function
            data_loader = DataLoader(list(zip(pred_images, gt_images)), collate_fn=self.collate_fn, batch_size=batch_size, shuffle=False, num_workers=4, pin_memory=True) # type: ignore
        ...
        for batch_eval in tqdm(data_loader):
            if self.task_type == 'T2I':
                ...
                pred_images = [img.to('cuda', non_blocking=True) * 255 for img in pred_images]
                gt_images = [img.to('cuda', non_blocking=True) * 255 for img in gt_images]
