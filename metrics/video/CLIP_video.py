...
from metrics.video.viclip.simple_tokenizer import SimpleTokenizer as _Tokenizer
from metrics.video.viclip.viclip import ViCLIP
...
class ViCLIPCalculator(BaseMetric):
    ...
    def collate_fn(self, batch):
        if self.task_type == 'T2V':
            ...
            pred_frames = [sample_frames_from_video(video_path, num_samples=self.num_frames) for video_path in pred_videos]
            return pred_frames, captions
        else:
            pred_videos, gt_videos = zip(*batch)
            pred_frames = [sample_frames_from_video(video_path, num_samples=self.num_frames) for video_path in pred_videos]
            gt_frames = [sample_frames_from_video(video_path, num_samples=self.num_frames) for video_path in gt_videos]
    ...
    def calculate_score(self, batch, batch_size=64, update=True):
        if self.task_type == 'T2V':
            ...
            texts = batch['caption']
            data_loader = DataLoader(list(zip(pred_videos, texts)), collate_fn=self.collate_fn, batch_size=batch_size, shuffle=False, num_workers=16, pin_memory=True) # type: ignore
        else:
            pred_videos = batch['pred_video']
            gt_videos = batch['gt_video']
            data_loader = DataLoader(list(zip(pred_videos, gt_videos)), collate_fn=self.collate_fn, batch_size=batch_size, shuffle=False, num_workers=16, pin_memory=True) # type: ignore
            ...
            if self.task_type == 'T2V':
                ...
                for i, frames in enumerate(pred_frames):
                    if len(frames) == 0:
                        ...
                        continue
                    vid_tensor = frames2tensor(frames, fnum=self.num_frames, target_size=self.target_size, device=torch.device(self.device))
                    vid_feat = self.clip.get_vid_features(vid_tensor)
                    text_feat = self.clip.get_text_features(captions[i], self.tokenizer, {})
                    score = 100 * (vid_feat * text_feat).sum(axis=-1)
                    all_scores.append(float(score.item()))
            else:
                pred_frames, gt_frames = batch_eval
                for i, (pred_frames, gt_frames) in enumerate(zip(pred_frames, gt_frames)):
                    if len(pred_frames) == 0 or len(gt_frames) == 0:
                        all_scores.append(float('nan'))
                        continue
                    
                    pred_vid_tensor = frames2tensor(pred_frames, fnum=self.num_frames, target_size=self.target_size, device=torch.device(self.device))
                    gt_vid_tensor = frames2tensor(gt_frames, fnum=self.num_frames, target_size=self.target_size, device=torch.device(self.device))
