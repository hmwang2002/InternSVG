...
from metrics.video.FVD import FVDCalculator
from metrics.video.SSIM_video import SSIMVideoCalculator
from metrics.video.LPIPS_video import LPIPSVideoCalculator
from metrics.video.CLIP_video import ViCLIPCalculator
from metrics.video.DINO_video import DINOVideoCalculator
from metrics.video.PSNR_video import PSNRVideoCalculator
...
class InternSVGVideoMetrics:
    def __init__(self, config: VideoMetricsConfig, tokenizer_path: str):
        ...
        _registry: Dict[str, tuple[str, Callable[[], BaseMetric]]] = {
            'use_FVD': ('FVD', lambda: FVDCalculator(model_name='i3d', resolution=128, sequence_length=16)),
            'use_ViCLIP_T2V': ('ViCLIP-T2V', lambda: ViCLIPCalculator(
                model_name='viclip', 
                ckpt_path='sarena_ckpt/ViClip-InternVid-10M-FLT.pth',
                num_frames=8,
                target_size=(224, 224),
                task_type='T2V')),
            'use_ViCLIP_V2V': ('ViCLIP-V2V', lambda: ViCLIPCalculator(
                model_name='viclip', 
                ckpt_path='sarena_ckpt/ViClip-InternVid-10M-FLT.pth',
                num_frames=8,
                target_size=(224, 224),
                task_type='V2V')),
            'use_DINO_Video': ('DINO-Video', lambda: DINOVideoCalculator(num_frames=8, batch_size=64)),
            'use_SSIM_Video': ('SSIM-Video', lambda: SSIMVideoCalculator(num_frames=8)),
            'use_LPIPS_Video': ('LPIPS-Video', lambda: LPIPSVideoCalculator(num_frames=8, batch_size=32)),
